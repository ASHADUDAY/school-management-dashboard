...
export class UserAuthService {
  ...
  loginUser(_loginData: any): Observable<any> {
    //payload and api-call are mocked for now, will be replaced with actual ones once we have them
    // _loginData = {
    //   username: "export",
    //   password: "yes",
    //   channelId: 0,
    ...
    // );
    return this.http.get<any>("../assets/mock/login/auth-login.json", {
      observe: "response",
    });
  }
  ...
  loginUser_2(_loginData: any) {
    return this.http
      .post(environment.apiUrlReq + "authenticate", _loginData, {
        observe: "response",
      })
      .subscribe((response) => {
  ...
  getResponseData(_loginData: any) {
    ...
    return this.http
      .post(url, _loginData, { headers: hdrs, observe: "response" })
      .subscribe((res) => {
        console.log("123", res);
  ...
  loginUserforToken(_loginData: any) {
    ...
    this.http
      .post<any>(environment.apiUrlReq + "authenticate", _loginData, {
        observe: "response",
      })
      .subscribe((resp) => {
  ...
  getAuthorised(id) {
    ...
    return this.http.get(
      environment.apiUrlReq + `GetAccessbyUserid?userId=${id}`,
      { observe: "response" },
    );
  }
  ...
  setRFQUsersList(params) {
    this.http;
    return this.http.get(
      environment.setRFQApiUrlReq +
        "bffdropdown/getLeSettingByGroupAndDesc" +
        "?legalentityid=" +
  ...
  azureAuthLogin(_loginData: any) {
    return this.http.post(
      environment.apiUrlReq + "authorizationcode",
      { Code: _loginData },
      {
        observe: "response",
      },
