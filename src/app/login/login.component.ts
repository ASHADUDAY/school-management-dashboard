...
export class LoginComponent implements OnInit {
        ...
        (res: any) => {
          ...
          if (this.loginData.length <= 0) {
            ...
            localStorage.setItem(
              "session",
              JSON.stringify(this.loginData.loginname),
            );
  ...
  checkAuthorisation(userId) {
    this.authService.getAuthorised(userId).subscribe(
      (res: any) => {
        console.log(res);
        if (res.status == 200) {
          if (res.body.data.responseObj1.response) {
            sessionStorage.setItem(
              "permission",
              JSON.stringify(res.body.data.responseObj1.response),
            );
            if (sessionStorage.getItem("officeId")) {
              let data =
                res.body.data.responseObj1.response?.userOfficeList.find(
                  (e) => e.office_id == sessionStorage.getItem("officeId"),
                );
              if (data) {
  ...
  LoginThroughSSO() {
    const url =
      "https://dev-api-prefiler.allcargoapps.com/prefiler-bff/prefiler-auth-bff/ad-signin"; // ✅ Replace with your API URL
    ...
    this.rawHttp.get(url, { headers }).subscribe({
      next: (response: any) => {
        if (response) window.location.href = response.authorizeUrl;
      },
      error: (error) => {
  ...
  sendAuthorizationCode() {
    ...
    this.authService.azureAuthLogin(this.code).subscribe(
      (res: any) => {
